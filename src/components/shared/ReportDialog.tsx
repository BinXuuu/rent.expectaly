...
const CATEGORY_OPTIONS: { value: ReportCategory; label: string }[] = [
  ...
  { value: "fraud_suspicion", label: "欺诈嫌疑" },
  { value: "duplicate", label: "重复发布" },
  { value: "already_rented", label: "房源已出租但未下架" },
  { value: "abusive_content", label: "辱骂/违规评论内容" },
  { value: "spam", label: "广告/垃圾信息" },
  { value: "other", label: "其他" },
];

/**
 * 通用举报入口：房源/评论均可复用。第一期为界面交互演示，
 * 提交后仅本地展示确认提示，尚未接入 reports 表的真实写入（对应 Stage 07 内容审核后台处理流程）。
 */
