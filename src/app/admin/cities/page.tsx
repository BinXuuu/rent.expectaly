...
export default async function AdminCitiesPage() {
  ...
  if (!profile || !can(profile.roles, "city:manage")) {
    return (
      <div>
  ...
  const citiesResult = await cityRepository.findAll();
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-ink-muted mt-1 text-sm">
          共 {cities.length} 个城市。新增/编辑操作将在接入真实数据库后开放。
        </p>
      </div>
            ...
            >
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-ink text-sm font-medium">{city.name["zh-CN"]}</span>
